...
/* channel marker: a hairline tick strip, echoing a signal trace */
.sd-head::after {{
    content: "";
    position: absolute; left: 0; right: 0; bottom: 0; height: 2px;
    background: linear-gradient(90deg,
        {C_OK} 0%, {C_GOOD} 22%, {C_WARN} 48%,
        {C_HIGH} 68%, {C_BAD} 86%, {C_EXTREME} 100%);
    opacity: 0.85;
}}
.sd-eyebrow {{
...
<div class="sd-head">
  ...
  <p>Reads every round's fixes for a date range, measures the delay between
  the time a device recorded a position and the time the server received it,
  and maps where on the course that delay grows.</p>
</div>
...
m1.metric("Median delay", f"{df['diff_s'].median():.0f}s",
          help=f"Target is {IDEAL_S:.0f}s or under.")
m2.metric("95th percentile", f"{df['diff_s'].quantile(0.95):.0f}s")
m3.metric("Worst delay", fmt_dur(df["diff_s"].max()))
m4.metric(f"Within {IDEAL_S:.0f}s (ideal)", f"{pct_ideal:.1f}%")
m5.metric(f"Within {OK_S:.0f}s (comfortable)", f"{pct_ok:.1f}%")
