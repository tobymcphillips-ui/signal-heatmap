...
def build_category_map(pts, mode, colour_by, bin_m, show_handovers,
    ...
    if mode == "trace":
        ...
        for rid, grp in pts.groupby("round_id", sort=False):
            ...
            fg = folium.FeatureGroup(name=f"Tag {grp['tag'].iloc[0]}")
...
def render_buffered_view(df):
    ...
    if mode == "aggregate":
        ...
        line_weight = o1.slider("Line weight", 2, 8, 4, key="buf_lw")
        show_marks = o2.checkbox("Mark switches", value=True,
                                 key="buf_marks",
    ...
    st.markdown(chips, unsafe_allow_html=True)
    if mode == "aggregate":
        st.caption("Solid = every fix in that square arrived the same way. "
                   "Dashed = a mix, so buffering starts or ends there.")
    ...
    with t2:
        ...
        by_l["pct_buffered"] = (100 * by_l["buffered_fixes"]
                                / by_l["fixes"]).round(2)
        by_l = by_l.sort_values("buffered_fixes", ascending=False)
        show_table(by_l)
        holes = sel.dropna(subset=["hole"])
...
MEASURES = {
    "Transmission delay": {
        "col": "diff_s",
        "noun": "delay",
        "blurb": "How long the server took to receive a fix after the "
                 "device recorded it.",
        "ideal": 2.0, "ok": 5.0, "max": 120.0,
    },
    "Recording gap": {
        "col": "interval_s",
        "noun": "gap",
        "blurb": "How long the device went between recording one fix and "
                 "the next.",
        "ideal": None, "ok": None, "max": 120.0,
    },
}
with _sw2:
    _measure = st.radio(
        "Measure", list(MEASURES), horizontal=True,
        label_visibility="collapsed", key="measure_mode",
        help="Delay shows how late fixes arrived. Gap shows how long the "
             "device went quiet. Both rise where signal is weak.")
MEA = MEASURES[_measure]
metric_col = MEA["col"]
NOUN = MEA["noun"]
st.caption(f"Showing **{_measure.lower()}** — {MEA['blurb'][0].lower()}"
           f"{MEA['blurb'][1:]}")
