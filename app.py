...
/* Metrics as quiet bordered cards rather than floating numbers */
...
[data-testid="stMetricValue"] { font-size: 30px; line-height: 1.1; }

/* Tabs: flat, with a green underline on the active one */
...
[data-testid="stTabs"] [aria-selected="true"] { color: #FFFFFF; }
[data-testid="stTabs"] [data-baseweb="tab-highlight"] {
    background-color: #89BF40;
}

/* Expanders and forms: subtle containers, no heavy chrome */
...
/* Section rules: thinner and less shouty than the default */
hr, [data-testid="stDivider"] hr {
    border-color: rgba(255,255,255,0.09) !important;
    margin: 1.1rem 0 !important;
}

/* Small caps label used above control groups */
.tm-label {
    font-family: 'Barlow Condensed', sans-serif;
    font-weight: 600;
    text-transform: uppercase;
...
/* Dataframes: lose the boxy border */
[data-testid="stDataFrame"] { border-radius: 8px; overflow: hidden; }

/* Keep the map flush inside its column */
...
@st.fragment
def render_map_panel(fdf, metric_col, thresh):
    """Map lives in its own fragment: changing a display setting redraws
    only this panel, not the analytics below or the fetch above."""
    ...
    st.caption(f"{{len(bad):,}} of {{len(fdf):,}} fixes meet the threshold "
               f"({{thresh:,}}s+) and are drawn below.")
...
with right:
    render_map_panel(fdf, metric_col, thresh)
...
# =================================================================
@st.fragment
def render_analytics(fdf, HAS_SIM):
    """Analytics in its own fragment: changing the focus band or a
    tab reruns only this section, leaving the map untouched."""
    ...
    if FOCUSED:
        ...
        if adf.empty:
            st.warning(f"No fixes fall in **{focus_label}** — "
                       "widen the focus.")
            return
    ...
    def group_rates(key: str) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            "within_max_pct": g.apply(within_max),
    ...
    with t_round:
        ...
        rnd = rnd.sort_values(["date", "_k", "start"]).drop(columns="_k")
        for _c in rnd.select_dtypes("number").columns:
            rnd[_c] = rnd[_c].round(1)
    ...
    with t_out:
        ...
        if out.empty:
            ...
            log = (out.sort_values("interval_s", ascending=False)
                   ...
                   .head(500))
            for _c in log.select_dtypes("number").columns:
                log[_c] = log[_c].round(1)
