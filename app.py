...
over_ok = int((df["diff_s"] > OK_S).sum())
...
m5.metric(f"Over {OK_S:.0f}s", f"{over_ok:,}",
          help="Fixes that arrived later than your acceptable threshold.")
m6.metric("Coverage health", f"{overall_health:.1f}%",
...
t_dist, t_hole, t_loc, t_round, t_dev, t_time, t_out = st.tabs(
    ["Delay distribution", "By hole", "By location", "By round",
...
with t_dist:
    ...
    if d.empty:
        ...
        # Bands built around your standard: ≤2s ideal, ≤5s fine, >5s not.
        bins = [0, 1, 2, 5, 10, 30, 60, 120, 300, float("inf")]
        labels = ["0–1s", "1–2s", "2–5s", "5–10s", "10–30s", "30–60s",
                  "1–2m", "2–5m", "5m+"]
        band_ok = {"0–1s": "ideal", "1–2s": "ideal", "2–5s": "acceptable"}
        ...
        dist = pd.DataFrame({
            ...
            "grade": [band_ok.get(b, "over threshold") for b in labels],
        })
        ...
        cc1.markdown(
            ...
            f"Green is inside your {OK_S:.0f}s threshold.")
        ...
        bars = base.mark_bar(cornerRadiusTopLeft=2,
            ...
            color=alt.Color("grade:N", scale=alt.Scale(
                domain=["ideal", "acceptable", "over threshold"],
                range=[TM_GREEN, "#B7D67A", TM_ORANGE]), legend=None),
...
with t_hole:
    ...
    if hs.empty:
        ...
        st.markdown(f"**Percentage of fixes arriving later than "
                    f"{OK_S:.0f}s, hole by hole** (holes in playing order)")
        st.altair_chart(
            bar_labeled(ch[["Hole", "pct_delayed", "fixes", "p95_diff_s"]],
                        "Hole", "pct_delayed", x_title="Hole",
                        y_title=f"% of fixes over {OK_S:.0f}s",
                        sort=order, color=TM_ORANGE, fmt=".1f", height=300),
            use_container_width=True)

        worst = hole_tbl["pct_delayed"].dropna().nlargest(3)
        if not worst.empty and worst.iloc[0] > 0:
            st.markdown("Worst holes: " + " · ".join(
                f"**Hole {int(h)}** ({v:.1f}% delayed)"
                for h, v in worst.items()))
        st.dataframe(hole_tbl.sort_values("pct_delayed", ascending=False),
                     use_container_width=True)
...
with t_loc:
    ...
    ranked = (loc[loc["fixes"] >= 20]
              .sort_values("pct_delayed", ascending=False).head(15))
    if ranked.empty:
        ranked = loc.sort_values("pct_delayed", ascending=False).head(15)
    cl = ranked.reset_index()
    st.markdown(f"**Worst 15 spots by share of fixes over {OK_S:.0f}s** "
                "(sorted worst first, 20+ fixes only)")
    st.altair_chart(
        bar_labeled(cl[["location", "pct_delayed", "fixes"]],
                    "location", "pct_delayed", x_title="",
                    y_title=f"% over {OK_S:.0f}s",
                    sort=list(cl["location"]), color=TM_ORANGE,
                    fmt=".1f", height=340, label_angle=-35),
        use_container_width=True)
    st.dataframe(loc.sort_values("pct_delayed", ascending=False),
                 use_container_width=True)
...
with t_round:
    ...
    rnd = (fdf.groupby(["round_id", "tag", "device_id"])
           .agg(date=("date", "first"),
                start=("recorded", "min"),
                end=("recorded", "max"),
                fixes=("fix_id", "count"),
...
with t_time:
    ...
    if hrs.empty:
        ...
        if fdf["date"].nunique() > 1:
            per_day = (fdf.dropna(subset=["date"]).groupby("date")
                       .agg(fixes=("fix_id", "count"),
                            pct_delayed=("diff_s", delayed_rate))
                       .round(1).reset_index())
