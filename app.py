...
def render_network_view(df):
    ...
    with t2:
        ...
        show_table(cells, use_container_width=True, hide_index=True)
...
def render_buffered_view(df):
    ...
    scope = c1.selectbox(
        "Scope", ["Single round", "Selected rounds", "Selected tags",
                  "All rounds (aggregate)"], key="buf_scope",
        help="A single round shows where buffering happened as the cart "
             "travelled. The aggregates pool many rounds onto one map.")
...
_sw1, _sw2 = st.columns([1.25, 1])
with _sw1:
    _view = st.radio(
        "View", ["Delay diagnostics", "Network coverage", "Buffered fixes"],
