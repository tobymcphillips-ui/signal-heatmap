...
LABELS = {
    ...
    "pct_delayed": "Over 5s %", "pct_of_group": "In focus %",
...
NET_PALETTE = ["#4C8DFF", "#F4A261", "#8AC926", "#E76F51", "#B15DE8",
               "#2EC4B6", "#FFD166", "#FF6B9D", "#7B8CDE", "#06D6A0",
               "#EF476F", "#C9A227"]
...
def build_network_map(pts, mode, colour_by, bin_m, show_handovers,
                      center_lat, center_lon, line_weight):
    """Render the network map to HTML. Cached like the delay map so an
    unrelated rerun doesn't rebuild it."""
    ...
    cmap = net_colours(pts[colour_by].dropna().unique())
    ...
    if mode == "trace":
        ...
        for rid, grp in pts.groupby("round_id", sort=False):
            ...
            for val, i0, i1 in runs_of(labels):
                ...
                if len(seg) < 2:
                    continue
                folium.PolyLine(
                    seg, color=cmap.get(val, "#9AA3AA"),
                    weight=line_weight, opacity=0.9,
                    popup=folium.Popup(
            ...
            if show_handovers:
                for i in range(1, len(labels)):
                    if labels[i] != labels[i - 1]:
                        folium.CircleMarker(
                            coords[i], radius=6, color="#FFFFFF", weight=2,
        ...
        for (_, _), grp in pts.groupby(["_bx", "_by"], sort=False):
            ...
            folium.CircleMarker(
                ...
                radius=max(3, min(9, 3 + len(grp) ** 0.4)),
                color=cmap.get(top, "#9AA3AA"),
                weight=1 if share >= 0.6 else 2,
                dash_array=None if share >= 0.6 else "3",
                fill=True, fill_color=cmap.get(top, "#9AA3AA"),
                fill_opacity=0.35 + 0.55 * share,
                popup=folium.Popup(
...
_view = st.radio(
    "View", ["Delay diagnostics", "Network coverage"],
