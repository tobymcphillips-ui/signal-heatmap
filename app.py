...
def render_network_view(df):
    ...
    if not have.any():
        st.info(
            "**No network data in these fixes yet.** The dashboard is "
            "expected to start populating the CellID, MCC/MNC and LAC "
            "columns — this view reads `cellid`, `mcc/mnc` and `lac` "
            "straight from the fixes API and will fill in on its own once "
            "they arrive. Nothing to configure.")
    ...
    if scope == "Single round":
        ...
        mode = c3.radio("Draw as", ["trace", "aggregate"], index=0,
                        help="Trace draws each round's path; aggregate "
                             "pools them into a dominance map.")
        ...
        mode = c3.radio("Draw as", ["aggregate", "trace"], index=0)
    else:
        sel = ndf
        mode = "aggregate"
    ...
    if mode == "aggregate":
        st.caption("Solid squares are places every device agreed on the "
                   "same operator. Dashed outlines mark contested squares "
                   "(under 60% agreement) — those are the handover zones, "
                   "and usually the marginal-coverage ones.")
    ...
    with t1:
        ...
        st.dataframe(by_net, use_container_width=True)
        ...
        if not holes.empty:
            ...
            st.markdown("**Fixes per network, hole by hole**")
    ...
    with t2:
        ...
        st.caption("A cell serving only a handful of fixes across many "
                   "rounds is usually a fringe cell the device grabs at the "
                   "edge of coverage.")
