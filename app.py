...
with right:
    ...
    # st_folium caches its component against the key it was given. With a
    # fixed key it will keep showing a stale map when only the data behind
    # it changed, so the key has to move with the inputs.
    _fp = (len(fdf), len(bad), metric_col, thresh, spread, log_weight,
           radius, blur, round(intensity, 2), show_markers, n_markers,
           show_lines, tuple(sorted(map(str, sel_tags))),
           tuple(sorted(map(str, sel_sims))) if HAS_SIM else (),
           round(num(fdf["lat"].sum()), 4), round(num(fdf["lon"].sum()), 4))
    st_folium(m, height=650, use_container_width=True,
              returned_objects=[],
              key="map_" + hashlib.md5(repr(_fp).encode()).hexdigest()[:12])
