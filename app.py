...
def build_category_map(pts, mode, colour_by, bin_m, show_handovers,
                       ...
                       dim_imagery, label_singular="change"):
    ...
    if dim_imagery:
        m.get_root().header.add_child(folium.Element(
            "<style>.leaflet-tile-pane{filter:brightness(0.45) "
            "saturate(0.55);}</style>"))
    ...
    if mode == "trace":
        ...
        for rid, grp in pts.groupby("round_id", sort=False):
            ...
            for val, i0, i1 in runs_of(labels):
                ...
                folium.PolyLine(seg, color="#08131B",
            ...
            if show_handovers:
                for i in range(1, len(labels)):
                    if labels[i] != labels[i - 1]:
                        ...
                        folium.CircleMarker(
                            coords[i], radius=7, color="#08131B", weight=3,
        ...
        for (_, _), grp in pts.groupby(["_bx", "_by"], sort=False):
            ...
            folium.CircleMarker(
                ...
                color="#08131B" if share >= 0.6 else "#FFFFFF",
            ...
            if not got.empty:
                ...
                for r in got.itertuples():
                    ...
                    folium.CircleMarker(
                        ...
                        color="#08131B", weight=2, fill=True,
                ...
                folium.CircleMarker(
                    ...
                    radius=6, color="#08131B", weight=2, fill=True,
...
def render_buffered_view(df):
    ...
    with st.spinner("Drawing map…"):
        html = build_category_map(
            ...
            num(sel["lat"].mean()), num(sel["lon"].mean()), line_weight,
            PAL, dim_imagery, label_singular="switch")
    components.html(html, height=660, scrolling=False)
...
def build_accuracy_map(pts, center_lat, center_lon, exact, dim_imagery,
    ...
    ramp = [(0.00, C_OK), (0.25, C_GOOD), (0.50, C_WARN),
    ...
    for rid, grp in pts.groupby("round_id", sort=False):
        ...
        coords = list(zip(grp["lat"], grp["lon"]))
        if len(coords) > 1:
            folium.PolyLine(coords, color="#FFFFFF", weight=1,
