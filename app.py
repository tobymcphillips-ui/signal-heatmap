...
# ------------------------------------------------------- interface styling
# Applies the Apple design language (DESIGN.md) in its documented dark-tile
# expression: product-tile surfaces, true-black nav, and Sky Link Blue as
# the single interactive colour. Chrome recedes; surface change is the
# divider; no gradients, no shadows on chrome.
#
# Tokens: colours, radii and spacing come straight from the spec.
C_ACCENT = "#2997ff"        # primary-on-dark — the one interactive colour
C_ACCENT_FOCUS = "#0071e3"  # primary-focus, keyboard ring only
C_BG = "#272729"            # surface-tile-1
C_SURFACE = "#2a2a2c"       # surface-tile-2 — micro-step for panels
C_SURFACE_DEEP = "#252527"  # surface-tile-3 — bottom of the stack
C_BLACK = "#000000"         # surface-black — nav only
C_TEXT = "#ffffff"          # on-dark
C_MUTED = "#cccccc"         # body-muted
C_HAIRLINE = "rgba(255,255,255,0.14)"

# Severity ramp. The spec forbids a second *interactive* accent; these are
# data encodings, not chrome, and the document covers no charting surface.
C_OK = "#30d158"
C_GOOD = "#9ee37d"
C_WARN = "#ffd60a"
C_HIGH = "#ff9f0a"
...
SF_DISPLAY = ('"SF Pro Display", system-ui, -apple-system, '
              'BlinkMacSystemFont, sans-serif')
SF_TEXT = ('"SF Pro Text", system-ui, -apple-system, '
           'BlinkMacSystemFont, sans-serif')

st.markdown(f"""
<style>
/* ---- Type. SF Pro Text for body at 17px/1.47/-0.374px; SF Pro Display
   with negative tracking for headlines. The boundary is unbreakable, so
   numerals stay in SF and use tabular figures for column alignment. ---- */
...
/* Captions and help text take typography.caption (14px, muted) — without
   this they inherit body size and colour and compete with real copy. */
...
h1 {{ font-size: 40px; line-height: 1.1; letter-spacing: -0.28px; }}
h2 {{ font-size: 34px; line-height: 1.15; letter-spacing: -0.374px; }}
h3 {{ font-size: 21px; line-height: 1.19; letter-spacing: 0.231px; }}
[data-testid="stMetricValue"], [data-testid="stDataFrame"] {{
    font-variant-numeric: tabular-nums;
}}

.stApp {{ background: {C_BG}; overflow-x: hidden; }}
.block-container {{
    padding-top: 48px; padding-bottom: 80px;
    max-width: 1440px;
}}

/* ---- Header. A full-bleed tile: no border, no radius, no gradient —
   the surface change is the divider. ---- */
.sd-head {{
    background: {C_SURFACE_DEEP};
    /* Full-bleed without overflow: pull out to the viewport edge by
       exactly the gutter width, then pad the same amount back in.
       (±100vw would make the element ~200vw wide and force a
       horizontal scrollbar.) */
    margin-left: calc(50% - 50vw);
    margin-right: calc(50% - 50vw);
    margin-top: -48px;
    margin-bottom: 32px;
    padding: 48px calc(50vw - 50%) 40px calc(50vw - 50%);
}}
.sd-head-row {{
...
.sd-head h1 {{
    ...
    letter-spacing: -0.28px; margin: 0; color: {C_TEXT};
}}
.sd-head p {{
    color: {C_MUTED};
    margin: 12px 0 0 0;
...
.sd-scale {{ flex: 0 0 auto; min-width: 280px; }}
.sd-scale-cap {{
    font-size: 12px; font-weight: 400; letter-spacing: -0.12px;
    color: {C_MUTED}; margin-bottom: 8px;
}}
.sd-scale-bar {{ display: flex; gap: 3px; }}
.sd-scale-bar i {{ display: block; height: 6px; flex: 1 1 0;
                   border-radius: 9999px; }}
.sd-scale-ticks {{
    display: flex; justify-content: space-between;
    font-size: 12px; color: {C_MUTED}; margin-top: 6px;
    letter-spacing: -0.12px;
}}

/* ---- Metrics use the store-utility-card grammar: 18px radius, hairline
   border, 24px padding, no shadow. ---- */
...
/* ---- Tabs: nav-link grammar, single accent marks the active channel ---- */
...
[data-testid="stTabs"] [data-baseweb="tab-highlight"] {{
    background-color: {C_ACCENT}; height: 2px;
}}

/* ---- Containers: utility-card radius, hairline, no shadow ---- */
...
/* ---- Buttons. Two grammars only: blue pill for actions, compact rect
   for utility. scale(0.95) on press is the system micro-interaction. ---- */
...
.stButton button[kind="secondary"] {{
    ...
    border: 1px solid {C_HAIRLINE} !important;
...
.stButton button[kind="secondary"]:active {{ transform: scale(0.95); }}
.stButton button:focus-visible, .stDownloadButton button:focus-visible {{
    outline: 2px solid {C_ACCENT_FOCUS} !important;
    outline-offset: 2px;
}}

/* ---- Sidebar is the nav surface: true black, the only pure black. ---- */
[data-testid="stSidebar"] {{
    background: {C_BLACK};
    border-right: 1px solid {C_HAIRLINE};
}}
...
/* ---- Inputs take the search-input grammar: pill, 44px, hairline ---- */
...
/* Single-value selects read as controls and take the pill; the tag
   multiselect holds chips and takes the utility radius instead, or a long
   tag list bulges awkwardly inside a pill. */
...
/* Divider: a hairline, used sparingly — surface change is preferred */
hr, [data-testid="stDivider"] hr {{
    border-color: {C_HAIRLINE} !important;
...
/* Links use the on-dark accent — Action Blue would disappear here */
a, a:visited {{ color: {C_ACCENT}; }}
...
def build_map_html(bad, ctx_pts, center_lat, center_lon, metric_col,
    ...
    if heat_points:
        ...
        HeatMap(
            ...
            max_zoom=19,
            gradient={0.10: "#248a3d", 0.30: C_OK, 0.50: C_GOOD,
...
with t_dev:
    ...
    st.altair_chart(
        bar_labeled(ct[dcols], "tag", d_y, x_title="Tag",
                    ...
                    sort=list(ct["tag"]),
                    color=TM_ORANGE if FOCUSED else C_ACCENT,
                    fmt=".1f", height=300),
        use_container_width=True)
    ...
    with t_sim:
        ...
        st.altair_chart(
            bar_labeled(cs[["sim_provider", "health_pct", "fixes", "tags"]],
                        ...
                        color=C_ACCENT, fmt=".1f", height=300),
            use_container_width=True)
        st.dataframe(
